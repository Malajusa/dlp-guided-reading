...
1. Weekly Blueprint Resolver establishes the authoritative Reading Focus, Learning Intention, Success Criteria, curriculum context, conceptual spine, factual source record, and visual constraints.
2. Central Passage Writer drafts five distinct Alpha-Epsilon rewrites from the same blueprint.
3. Five Level Pitch Review Agents inspect their assigned passage independently and in parallel:
   - Alpha Pitch Agent
   - Beta Pitch Agent
   - Gamma Pitch Agent
   - Delta Pitch Agent
...
5. The revised passage returns to the same level agent. A passage cannot advance without `PASS`.
6. When all five passages pass individually, a Progression and Parity Agent compares them side-by-side.
7. The existing question-design/evidence QA then operates on the approved texts.
...
A reviewer must evaluate the final wording it is asked to approve, not a summary of the passage.
...
It cannot lower or raise teacher-assigned group placements.
...
- response depth expected by the attached question plan;
...
- the text is essentially Alpha with a few harder words;
- the text is functionally Gamma-level without adequate support;
- difficulty is added mainly through vocabulary substitution;
- evidence remains so immediate that Beta has no meaningful progression from Alpha.
...
- Gamma is too close to Beta or Delta to represent a defensible centre point;
...
- reasoning demand is not materially greater than Gamma;
...
- the passage is simply longer than Delta;
...
- genre/text form;
- planned question metadata, including evidence distance and response depth;
- visual plan for that group;
- relevant group-profile rules;
- any explicit teacher-supplied point-of-need notes.

The agent does not receive permission to change placement or redesign the weekly sequence.
...
`PASS` is invalid if `blocking_issues` or `required_revisions` is non-empty.
...
Any revision that changes evidence locations, answerability, or wording relied on by questions invalidates dependent question records and sends them back through question/evidence QA.
...
- each level agent initially reviews only its own passage, not neighbouring texts;
- level agents do not see other reviewers' verdicts before issuing their own;
- only the Progression and Parity Agent receives all five approved passages together;
...
- any level lacks a `PASS` from its assigned agent;
- the Progression and Parity Agent does not return `PASS`;
...
3. Beta passage that is merely Alpha plus harder words — Beta agent must reject/revise.
4. Gamma passage simplified below Year 4/5 demand — Gamma agent must reject/revise.
5. Delta passage that is only longer than Gamma — Delta agent must reject/revise.
...
- no passage can reach question/layout generation without required approvals;
